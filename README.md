...
- **Daftar item** — tabel semua item dengan pencarian, klik baris untuk edit. Centang satu/beberapa baris (atau "Pilih semua") lalu klik **Hapus item terpilih** untuk menghapus sekaligus — otomatis tercatat di log.
- **Edit item** — perubahan otomatis tercatat sebagai log.
- **Histori perbaikan** — di dalam modal edit, ada kolom "Perlakuan" untuk mencatat perbaikan/pembaruan pada item (misal: ganti komponen, servis, kalibrasi). Setiap kali disimpan, catatan itu langsung muncul di daftar "Histori Perbaikan" di bawahnya, dan tersimpan rapi di sheet terpisah bernama **Histori Perbaikan** (kolom: Tanggal, Jam, ID Item, Nama Item, Perlakuan) — mudah difilter/di-sort langsung dari Google Sheets.
- **Log aktivitas** — tanggal, jam, dan apa yang diubah, tersimpan permanen di Google Docs.

> ⚠️ **Penting:** setiap kali Anda mengganti isi `Code.gs`, perubahan **tidak otomatis aktif** di URL Web App yang sudah ada. Anda wajib membuat versi deployment baru — lihat langkah di bagian Setup #3. Kalau lupa, gejalanya persis seperti "field baru muncul di form tapi datanya tidak pernah tersimpan" karena backend lama masih berjalan.
>
> Cara cepat memastikan versi backend sudah ter-update: lihat status koneksi di sidebar kiri bawah. Sejak versi ini, statusnya menampilkan nomor versi backend (misalnya "Terhubung · v3.0.0") — bandingkan dengan `BACKEND_VERSION` di baris atas `Code.gs` Anda.
