...
function openEditModal(item) {
  ...
  document.getElementById('editPerlakuan').value = '';
  document.getElementById('perlakuanMsg').textContent = '';
  generateQrInto('editQrCanvas', item, 130);
  document.getElementById('editModalOverlay').hidden = false;
  loadPerlakuanHistory(item.id);
}

async function loadPerlakuanHistory(id) {
  const list = document.getElementById('historyList');
  list.innerHTML = '<li class="empty-row">Memuat histori…</li>';
  try {
    const res = await apiGet('getPerlakuanHistory', { id });
    if (res.success) {
      list.innerHTML = res.history.length
        ? res.history.map(h => `<li><span class="history-date">${escapeHtml(h.tanggal)} · ${escapeHtml(h.jam)}</span>${escapeHtml(h.perlakuan)}</li>`).join('')
        : '<li class="empty-row">Belum ada histori perbaikan untuk item ini.</li>';
    } else {
      list.innerHTML = '<li class="empty-row">' + escapeHtml(res.message || 'Gagal memuat histori.') + '</li>';
    }
  } catch (err) {
...
document.getElementById('btnSimpanPerlakuan').addEventListener('click', async () => {
  const id = document.getElementById('editId').value;
  const textarea = document.getElementById('editPerlakuan');
  const text = textarea.value.trim();
  const msg = document.getElementById('perlakuanMsg');
  const btn = document.getElementById('btnSimpanPerlakuan');

  if (!text) {
    msg.textContent = 'Isi perlakuan/perbaikan terlebih dahulu.';
    msg.className = 'form-msg err';
    return;
  }
  ...
  try {
    const res = await apiPost('addPerlakuan', { id, perlakuan: text });
    if (res.success) {
      textarea.value = '';
      msg.textContent = 'Perlakuan ditambahkan ke histori.';
      msg.className = 'form-msg ok';
      loadPerlakuanHistory(id);
      showToast('Histori perbaikan disimpan', 'ok');
    } else {
      msg.textContent = res.message || 'Gagal menyimpan perlakuan.';
      msg.className = 'form-msg err';
    }
  } catch (err) {
    msg.textContent = 'Tidak dapat terhubung ke backend.';
