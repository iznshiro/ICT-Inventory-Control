...
async function refreshItemList() {
  ...
  tbody.innerHTML = '<tr><td colspan="6" class="empty-row">Memuat data…</td></tr>';
  try {
    ...
    if (res.success) {
      ...
      tbody.innerHTML = '<tr><td colspan="6" class="empty-row">' + escapeHtml(res.message || 'Gagal memuat data.') + '</td></tr>';
    }
  } catch (err) {
    tbody.innerHTML = '<tr><td colspan="6" class="empty-row">Tidak dapat terhubung ke backend.</td></tr>';
  }
}
...
function renderItemTable(items) {
  const tbody = document.getElementById('itemTableBody');
  if (items.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" class="empty-row">Belum ada item. Tambahkan item pertama Anda.</td></tr>';
    return;
  }
